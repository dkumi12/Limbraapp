...
const ExerciseSearch = ({
  ...
  const handleSearch = async searchQuery => {
    const queryToUse =
      typeof searchQuery === 'string' ? searchQuery : searchTerm;
    ...
    try {
      ...
      const formattedRoutine = {
        ...
        totalDuration: routine.exercises.reduce(
          (sum, ex) => sum + ex.duration_seconds,
          0
        ),
        difficulty: difficulty,
        benefits: [...new Set(routine.exercises.flatMap(ex => ex.purpose))].map(
          p => `Improves ${p}`
        ),
  ...
  return (
    <div className="exercise-search" style={{ padding: '1rem 0' }}>
      ...
      <p style={{ marginBottom: '1rem', color: '#b0b8c9', fontSize: '0.9rem' }}>
        Enter a body part, condition, or specific goal to get a customized
        routine
      </p>
      ...
      {searchError && (
        <div
          style={{ color: '#ef4444', marginBottom: '1rem', fontSize: '0.9rem' }}
        >
          {searchError}
        </div>
      )}
      ...
      <div style={{ marginBottom: '1.5rem' }}>
        ...
        >
          <label style={{ fontSize: '0.9rem', color: '#e2e8f0' }}>
